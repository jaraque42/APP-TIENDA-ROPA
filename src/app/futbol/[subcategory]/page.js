...
export default function FutbolSubcategoryPage() {
  const params = useParams();
  const subcategory = params.subcategory;
  
  const futbolProducts = PRODUCTS.filter(p => {
    const isCategory = p.category === 'futbol';
  ...
  return (
    <div className="running-grid-area">
      <div className="product-grid running-grid">
        {futbolProducts.length > 0 ? (
          futbolProducts.map((product) => (
